...
/*
void Ip_length(string& tmp_loc_ip, int& tmp_length){
    fstream f_length;
		char tmp[50];
		strcpy(tmp, tmp_loc_ip.c_str());
		f_length.open(tmp, ios::in);
    string line;
	  if(!f_length)
		  cout << "Fail!" << endl;
	  else{
			int r, l;
		  vector<int> rl = vector<int>();
			do{
			  getline(f_length, line);
        string tmp_length_l = line;
        lst(tmp_length_l, "L 10 ", "L 10 ", " 10 0 ");
        string tmp_length_r = line;
        lst(tmp_length_r, "L 250 ", "L 250 ", " 10 0 ");
        if(!tmp_length_l.empty()){
          l = str2int(tmp_length_l);
					rl.push_back(l);
				}
        if(!tmp_length_r.empty()){
          r = str2int(tmp_length_r);
      		rl.push_back(r);
				}
			}while(!f_length.eof());
      f_length.close();
      if(rl.size() != 0){
			  vector<int>::iterator myMax = max_element(rl.begin(), rl.end());
    	  vector<int>::iterator myMin = min_element(rl.begin(), rl.end());
        tmp_length = *myMax-*myMin;
      }
      else 
        tmp_length = 0;
      rl.clear();
...
//  }
  ...
  cout << "  2. Show #IP/#PIN" << "       7. Format #SCH" << endl;
